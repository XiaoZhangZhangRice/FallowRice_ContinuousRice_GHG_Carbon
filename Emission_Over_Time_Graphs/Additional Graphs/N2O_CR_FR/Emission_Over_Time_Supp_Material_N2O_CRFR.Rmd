---
title: "Emissions Over Time - CR and FR (N2O)"
...
N2O_g_ha_day_2021_graphing <- CRFR_2021_22_rice %>% group_by(Date, Treatment) %>% 
mutate(N2O_g_ha_day_sd = sd(N2O_g_ha_day)) %>% 
  summarise(N2O_g_ha_day = mean(N2O_g_ha_day),
            N2O_g_ha_day_sd = mean(N2O_g_ha_day_sd))

gas_time_2021_graph <- ggplot(N2O_g_ha_day_2021_graphing, aes(x= Date, y = N2O_g_ha_day, color = Treatment))+
  geom_point()+
  geom_line()+
  geom_errorbar(aes(ymin=N2O_g_ha_day-N2O_g_ha_day_sd, ymax=N2O_g_ha_day+N2O_g_ha_day_sd))+
...
N2O_g_ha_day_2022_graphing <- CRFR_2022_23_rice %>% group_by(Date, Treatment) %>% 
mutate(N2O_g_ha_day_sd = sd(N2O_g_ha_day)) %>% 
  summarise(N2O_g_ha_day = mean(N2O_g_ha_day),
            N2O_g_ha_day_sd = mean(N2O_g_ha_day_sd))

gas_time_2022_graph <- ggplot(N2O_g_ha_day_2022_graphing, aes(x= Date, y = N2O_g_ha_day, color = Treatment))+
  geom_point()+
  geom_line()+
  geom_errorbar(aes(ymin=N2O_g_ha_day-N2O_g_ha_day_sd, ymax=N2O_g_ha_day+N2O_g_ha_day_sd))+
  scale_color_manual(values = c("CR" = "#0072B2", "FR" = "#FFCC66"),labels = c("Continuous rice (CR)", "Fallow rice (FR)"))+
  labs(y = expression("Daily N2"[4]~"O Flux (g ha"^-1~" day"^-1~")")) +
  scale_y_continuous(limits = c(-200, 600), 
                     expand = c(0, 0),
                     )+
  ggtitle("2022/23") +
...
N2O_g_ha_day_2023_graphing <- summer_2023_rice %>% group_by(Date, Treatment) %>% 
mutate(N2O_g_ha_day_sd = sd(N2O_g_ha_day)) %>% 
  summarise(N2O_g_ha_day = mean(N2O_g_ha_day),
            N2O_g_ha_day_sd = mean(N2O_g_ha_day_sd))

gas_time_2023_graph <- ggplot(N2O_g_ha_day_2023_graphing, aes(x= Date, y = N2O_g_ha_day, color = Treatment))+
  geom_point()+
  geom_line()+
  geom_errorbar(aes(ymin=N2O_g_ha_day-N2O_g_ha_day_sd, ymax=N2O_g_ha_day+N2O_g_ha_day_sd))+
  scale_color_manual(values = c("CR" = "#0072B2", "FR" = "#FFCC66"),labels = c("Continuous rice (CR)", "Fallow rice (FR)"))+
