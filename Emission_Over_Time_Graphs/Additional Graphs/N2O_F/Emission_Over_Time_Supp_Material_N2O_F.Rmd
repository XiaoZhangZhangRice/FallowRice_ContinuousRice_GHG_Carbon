...
N2O_g_ha_day_2021_graphing <- F_2021_22 %>% group_by(Date) %>% 
mutate(N2O_g_ha_day_sd = sd(N2O_g_ha_day)) %>% 
  summarise(N2O_g_ha_day = mean(N2O_g_ha_day),
            N2O_g_ha_day_sd = mean(N2O_g_ha_day_sd))

gas_time_2021_graph <- ggplot(N2O_g_ha_day_2021_graphing, aes(x= Date, y = N2O_g_ha_day))+
  geom_point()+
  geom_line()+
...
N2O_g_ha_day_2022_graphing <- F_2022_23 %>% group_by(Date) %>% 
mutate(N2O_g_ha_day_sd = sd(N2O_g_ha_day)) %>% 
  summarise(N2O_g_ha_day = mean(N2O_g_ha_day),
            N2O_g_ha_day_sd = mean(N2O_g_ha_day_sd))

gas_time_2022_graph <- ggplot(N2O_g_ha_day_2022_graphing, aes(x= Date, y = N2O_g_ha_day))+
  geom_point()+
  geom_line()+
  geom_errorbar(aes(ymin=N2O_g_ha_day-N2O_g_ha_day_sd, ymax=N2O_g_ha_day+N2O_g_ha_day_sd))+
  #scale_color_manual(values = c("#FFCC66"))+
